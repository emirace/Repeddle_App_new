...
const ThisWeek = () => {
  ...
  const from = useMemo(() => moment(today).endOf("day").toDate(), [today])
  const to = useMemo(() => getMonday(today), [today])
...
export function getMonday(d: Moment) {
  let ds = d.toDate()
  var day = ds.getDay(),
    diff = ds.getDate() - day + (day == 0 ? -6 : 1) // adjust when day is sunday
  return new Date(ds.setDate(diff))
}
