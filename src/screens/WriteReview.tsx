...
const WriteProductReview = ({
  ...
  const handleSubmit = async () => {
    ...
    if (params.item === "product") {
      ...
      const res = await reviewSeller(seller.user._id, {
        comment,
        rating: Number(rating),
        like,
      })
  ...
  const canReview = useMemo(() => {
    if (params.item === "product") {
      return user && product?.buyers.some((buyer) => buyer === user._id)
    } else {
      return true
      // return (
      //   user &&
      //   seller?.user.reviews.some((review) => review.user._id === user?._id)
      // )
    }
  }, [params.item, product, seller, user])
    ...
    !hasReviewed ? (
      <View style={styles.container}>
        ...
        >
          ...
          <Appbar.Content
            ...
            title={`Write a ${params.item} review`}
          />
        </Appbar.Header>
        <ScrollView style={styles.scrollView}>
          <TextInput
            label={hasReviewed ? "Edit your review" : "Write a review"}
            multiline
            numberOfLines={4}
            value={comment}
          ...
          <View>
            ...
            <View style={styles.like}>
              <IconButton
                ...
                iconColor={like ? colors.primary : colors.onBackground}
                mode="outlined"
              />
              <IconButton
                icon="thumb-down"
      ...
      <View style={styles.container}>
        <Text style={{ textAlign: "center" }}>
          You have already reviewed this {params.item}
        </Text>
      </View>
    )
  ) : (
